...
(function() {
	...
	describe('FeedbacksController', function() {
		...
		it('$scope.find() should create an array with at least one article object fetched from XHR', inject(function(Feedbacks) {
			// Create sample article using the Feedbacks service
			var sampleFeedback = new Feedbacks({
				title: 'An Feedback about MEAN',
				content: 'MEAN rocks!'
			});

			// Create a sample feedbacks array that includes the new article
		...
		it('$scope.findOne() should create an array with one article object fetched from XHR using a articleId URL parameter', inject(function(Feedbacks) {
			...
			// Test scope value
			expect(scope.article).toEqualData(sampleFeedback);
		}));
		...
		it('$scope.update() should update a valid article', inject(function(Feedbacks) {
			...
			scope.article = sampleFeedbackPutData;
		...
		it('$scope.remove() should send a DELETE request with a valid articleId and remove the article from the scope', inject(function(Feedbacks) {
			// Create new article object
			var sampleFeedback = new Feedbacks({
				_id: '525a8422f6d0f87f0e407a33'
			});

			// Create new feedbacks array and include the article
